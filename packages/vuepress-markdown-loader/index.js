...
module.exports = function (src) {
  ...
  const { sourceDir, extractHeaders: extractHeadersPattern = ["h2", "h3"] } =
    options;
  ...
  if (!isProd && !isServer) {
    ...
    if (
      ...
      (cachedData.inferredTitle !== inferredTitle ||
        JSON.stringify(cachedData.frontmatterData) !==
          JSON.stringify(frontmatter.data) ||
  ...
  links &&
    links.forEach((link) => {
      ...
      const shortname = link.replace(/#.*$/, "").replace(/\.html$/, ".md");

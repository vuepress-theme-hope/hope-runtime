...
const { frontmatterEmitter } = require("@mr-hope/vuepress-markdown-loader");
const webpack = require("webpack");
const chokidar = require("chokidar");

const { path, fs, logger } = require("@mr-hope/vuepress-shared-utils");
...
module.exports = class DevProcess extends EventEmitter {
  ...
  watchUserConfig() {
    ...
    this.configWatcher.on("change", (target) =>
  ...
  watchFrontmatter() {
    frontmatterEmitter.on("update", (target) =>
      this.handleUpdate("frontmatter", target)
    );
  }
  ...
  async resolvePort() {
    this.port = await resolvePort(
      this.context.options.port || this.context.siteConfig.port
    );
  }
    ...
    config.plugin("vuepress-log").use(DevLogPlugin, [
      {
        ...
        clearScreen: !(
          this.context.options.debug || !this.context.options.clearScreen
        ),
      },
    ]);
    ...
    const serverConfig = Object.assign(
      {
        ...
        before: (app, server) => {
          ...
          this.context.pluginAPI.applySyncOption(
            "beforeDevServer",
            app,
            server
          );

...
describe("markdown page", () => {
  ...
  test("should extract headers by config", async () => {
    ...
    const page = await setupPage(
      { filePath, relative, extractHeaders },
      { markdown }
    );
...
describe("enhancer", () => {
  test("should loop over sync enhancers", async () => {
    ...
    return enhancers.map((enhancer) =>
      expect(enhancer.value).toHaveBeenCalled()
    );
  });
  ...
  test("should loop over sync and async enhancers", async () => {
    ...
    return mixedEnhancers.map((enhancer) =>
      expect(enhancer.value).toHaveBeenCalled()
    );
  });
...
describe("public api", () => {
  ...
  test("slug", async () => {
    ...
    const indexPage = await setupPage(
      {
        filePath: indexPageFixture.filePath,
        relative: indexPageFixture.relative,
      },
      { markdown }
    );
  ...
  test("date", async () => {
    ...
    const { relative, filePath } = getDocument("2020-01-01-date.md");
    const markdown = getMarkdown();
    const dateInPathPage = await setupPage(
      { filePath, relative },
      { markdown }
    );

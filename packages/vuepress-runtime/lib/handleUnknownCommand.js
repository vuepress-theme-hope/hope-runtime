...
const { createApp } = require("@mr-hope/vuepress-core");
const {
  path,
  logger,
  globby,
...
module.exports = async function (cli, options) {
  ...
  const needPrepareBeforeLaunchCLI =
...
async function inferUserDocsDirectory(cwd) {
  const paths = await globby(
    ["**/.vuepress/config.js", "!**/node_modules/**"],
    {
      cwd,
      dot: true,
    }
  ...
  if (siteConfigPath) {
    return path.resolve(cwd, siteConfigPath.replace(".vuepress/config.js", ""));
  }
  return null;
}
